...
// TODO
// solve err.more being allocated sometimes and sometimes not.
...
format_error :: proc(err: Error, allocator := context.allocator) -> (message: string, fatal: bool) {
    descriptions : [ErrorType] string = {
        ...
        .Missing_Key        = "Expected the name of a key before '='",
...
validate_string :: proc() -> bool {//{{{
    ...
    validate_escapes :: proc() -> bool {
        return false
    }

    validate_codepoints :: proc() -> bool {
        return false
    }
    ...
    return validate_quotes() || validate_escapes() || validate_codepoints()
}//}}}
...
validate_number :: proc() -> bool {//{{{
    ...
    is_digit_seq :: proc(sequence: string, base: int) -> bool {
        for r in sequence {
            if !(is_digit(r, base) || r == '_') do return false
        }
        return true
    }
